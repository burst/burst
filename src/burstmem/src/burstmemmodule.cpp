...
#ifdef BURSTMEMMODULE_IS_REMOTE_OFF
...
#error "not in c++?"
  ...
  ALCALL int _createModule (ALPtr < ALBroker > pBroker)
  {
    ...
    ALModule::createModule < burstmem > (pBroker, "burstmem");
...
{
  std::cout << "Exiting BURSTMEMMODULE." << std::endl;
...
{
  std::cout << "..::: starting BURSTMEM module :::.." << std::endl;
  //std::cout << "Copyright (c) 2007, Aldebaran-robotics" << std::endl << std::endl;

  int i = 1;
  std::string brokerName = "burstmem";
  std::string brokerIP = "";
  int brokerPort = 0;
  ...
  try
  {
    ...
    ALModule::createModule < burstmem > (broker, "burstmem");
  }
  catch (ALError & e)
  {
    std::

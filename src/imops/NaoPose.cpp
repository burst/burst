...
void NaoPose::transform () {
...
#ifdef OFFLINE
  // Bug in webots right now - angleX is off by 90 degrees.
  // simply disabling it makes it right for upgright position, which
  // is correct most of the time in webots anyhow
  bodyInclinationX = 0.0;
  bodyInclinationY = 0.0;
#else
  ...
  //std::cout << "angleX = " << bodyInclinationX << ", angleY = " << bodyInclinationY << ", comHeight = " << comHeight << std::endl;
...
//  const float height_mm_right = intersectionRight(Z);
  ...
  const float height_mm_left = IMAGE_HEIGHT_MM/2;
  const float height_mm_right = IMAGE_HEIGHT_MM/2;
